	...
	function beans_no_widget()
	{
		...
		*/
		beans_open_markup_e('beans_no_widget_notice','p',array('class' => 'uk-alert uk-alert-warning'));
			...
			beans_output_e(	'beans_no_widget_notice_text',sprintf(esc_html__('%s does not have any widget assigned!','bex-uikit2'),beans_get_widget_area('name')));
	...
	*/
	function beans_widget_rss_content()
	{
		...
		*/
		...
		return '<p><a class="uk-button" href="' . beans_get('url',$options) . '" target="_blank">' . esc_html__('Read feed','bex-uikit2') . '</a><p>';
